...
def test_clean_customer_data():
    ...
    spark = SparkSession.builder \
        .appName("test_clean_customer_data") \
        .master("local[*]") \
        .getOrCreate()
    ...
    try:
        ...
        with patch('__main__.validate_data_quality', side_effect=lambda x: x):
            ...
            actual_result = actual_df.orderBy("customer_id").collect()
            expected_result = expected_df.orderBy("customer_id").collect()
            ...
            null_customer_ids = actual_df.filter(col("customer_id").isNull()).count()
            null_emails = actual_df.filter(col("email").isNull()).count()
