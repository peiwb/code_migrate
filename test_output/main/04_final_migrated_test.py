...
def test_main_successful_pipeline_execution():
    ...
    1. Snowpark session is created and properly cleaned up
    ...
    # TODO: [MANUAL MIGRATION REQUIRED] - Configure appropriate Snowflake connection parameters
    ...
    try:
        ...
        mock_customer_data = session.create_dataframe([
            ("C001", "John Doe", "john@email.com", 30),
            ("C002", "Jane Smith", "jane@email.com", 25)
        ], customer_schema)
        
        mock_transaction_data = session.create_dataframe([
        ...
        expected_enriched = session.create_dataframe([
        ...
        expected_insights = session.create_dataframe([
            ("C001", 250.0, 2, 125.0),
            ("C002", 200.0, 1, 200.0)
        ], insights_schema)
        
        # Mock all external function dependencies
        with patch('__main__.create_snowpark_session', return_value=session), \
